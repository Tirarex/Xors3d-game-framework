...
sampler sParalax = sampler_state
{
	Texture	  = <tParalax>;
	AddressU  = WRAP;
	AddressV  = WRAP;
...
void vs_PW_TBN_TC( in sv_TBN_TC IN, out sp_PW_TBN_TC OUT ) {
	...
	OUT.Depth     = OUT.vPos;
	//OUT.Depth     =  mul(mul(IN.vPos, g_mWorld), g_mWorldViewProj);
}
...
ps_OU NormalMapPS( in sp_PW_TBN_TC IN): COLOR0
 {
	...
	float3 CamPosPlx = normalize(mul(campos,matTangentSpace));
  	...
  	for(;fCurrHeight < fCurrentBound;fCurrentBound -= fStepSize)
   	{
    	...
    	fCurrHeight = tex2Dlod(sDiffuse, float4(vTexCurrentOffset.xy,0,0) ).r;
	}

    float4 offsetBest = float4(vTexCurrentOffset,0,0);
    vTexCurrentOffset.xy -= vTexOffsetPerStep;

  	float fPrevHeight = tex2D( sDiffuse, vTexCurrentOffset.xy ).r;
    ...
    { 
      ...
      offsetBest.xy = -t * intersect.xy + intersect.zw;
  	  ...
  	  float NB = tex2D(sDiffuse, offsetBest).r;
	...
	float3 vNormal = normalize(tex2D(sNormals, newcords) * 2.0f - 1.0f);
	...
	float4	cD = tex2D(sDiffuse,newcords);
	//clip(cD.a - 0.5f); // mask
	if (cD.a < 0.5f) discard;
	OUT.oCol = float4(cD.rgb,Luminance( tex2D(sSpecular, newcords) ));
    ...
    //OUT.oCol2.x     = depth;
   // OUT.oCol2.y    = Luminance( tex2D(sSpecular, IN.vTexCoords) )  ;
   // OUT.oCol2.z   =FarDepth;
    //OUT.oCol2.w		= Luminance( tex2D(sBright, IN.vTexCoords) )  ;
