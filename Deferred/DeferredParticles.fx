...
int EnableLight = 0;
 ...
 inline float3 GetLighting(  float3 pos)
{

	if ( EnableLight == 1 ) {
	    float3 LighTing=(0,0,0);
        for( int i = 0; i < (int)lightCount; i++ ) {           
            float distance = length( pos- lightPosition[i] );
            float attenuation= saturate(1.0f - length(distance)/lightRange[i].x);
            LighTing.xyz += lightColor[i].xyz * attenuation;
        }
...
float4 SoftParticlePS (	in VS_OUTPUT IN ) : COLOR 
{
		...
		float DepthDiff=GetDiffAlpha( IN.Depth,IN.P ,IN.ParDepth);	
		Color.xyz=Color.xyz*GetLighting(IN.Pso);
		Alpha=Alpha*Color.a;
...
float4 SoftParticlePS2 (	in VS_OUTPUT IN ) : COLOR 
{
...
float4 PSWater (	in VS_OUTPUT IN ) : COLOR 
{
		...
		Color.xyz=Color.xyz*GetLighting(IN.Pso);
		Color.rgb=lerp( Color.rgb, float3(0.1,0.1,0.1), Fog );
...
technique Soft { 
	pass p {	
		...
		PixelShader 	= compile ps_3_0 SoftParticlePS();
	}
}
...
destblend = One;
		VertexShader 	= compile vs_3_0 VS();
		PixelShader 	= compile ps_3_0 OnlyRenderPS();	
	}
}
